...
bool BaseCreature::kill_mobs_if_thrown(int dmg_to_apply) {
    ...
    if (abs(xSpeed) > 0 || abs(ySpeed) > 0) {
        for (unsigned long a = 0; a < global::creatures.size(); a++) {
            ...
            if ((global::creatures.at(a)->spritesheet_type == SpritesheetType::SNAKE ||
                 global::creatures.at(a)->spritesheet_type == SpritesheetType::BAT_JETPACK ||
                 global::creatures.at(a)->spritesheet_type == SpritesheetType::CAVEMAN_DAMSEL ||
                 global::creatures.at(a)->spritesheet_type == SpritesheetType::SKELETON_SPIDER)
                && !global::creatures.at(a)->killed) {
                if (Collisions::checkCollisionBodies(x, y, 16, 16, global::creatures.at(a)->x,
                                                     global::creatures.at(a)->y, physical_width, physical_height)) {
    ...
    if (abs(xSpeed) > 0 || abs(ySpeed) > 0) {
        for (unsigned long a = 0; a < global::creatures.size(); a++) {
            ...
            if ((global::creatures.at(a)->spritesheet_type == SpritesheetType::SNAKE ||
                 ...
                 global::creatures.at(a)->spritesheet_type == SpritesheetType::SKELETON_SPIDER ||
                 global::creatures.at(a)->spritesheet_type == SpritesheetType::CAVEMAN_DAMSEL ||
                 global::creatures.at(a)->spritesheet_type == SpritesheetType::JAR)
                && !global::creatures.at(a)->ready_to_dispose) {
                ...
                if (Collisions::checkCollisionBodies(x, y, physical_width, physical_height,
                    ...
                    global::creatures.at(a)->xSpeed += this->xSpeed * 0.3f;
...
void BaseCreature::apply_gravity(double acceleration) {
    if (!bottomCollision)
        ySpeed += acceleration;
}

//limit speed to its maximum
void BaseCreature::limit_speed(int max_x, int max_y) {
...
void BaseCreature::apply_friction(float friction_delta_speed) {
...
void BaseCreature::get_x_y_viewported(int *out_main_x, int *out_main_y, int *out_sub_x, int *out_sub_y) {
...
void BaseCreature::update_position() {
...
void BaseCreature::set_sprite_state_basing_on_speed() {
    if (xSpeed > 0)
