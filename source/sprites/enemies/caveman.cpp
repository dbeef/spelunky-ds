...
void Caveman::init() {
    ...
    frameGfx = (u8 *) gfx_cavemanTiles;
    subSpriteInfo->updateFrame(frameGfx, sprite_width * sprite_height);
    mainSpriteInfo->updateFrame(frameGfx, sprite_width * sprite_height);

    spriteType = SpritesheetType::CAVEMAN;
...
void Caveman::initSprite() {
    subSpriteInfo = global::sub_oam_manager->initSprite(gfx_cavemanPal, gfx_cavemanPalLen,
                                                        nullptr, sprite_width * sprite_height, 16, CAVEMAN, true,
                                                        false,
                                                        LAYER_LEVEL::MIDDLE_BOT);
    mainSpriteInfo = global::main_oam_manager->initSprite(gfx_cavemanPal, gfx_cavemanPalLen,
                                                          nullptr, sprite_width * sprite_height, 16, CAVEMAN, true,
...
void Caveman::apply_stunned_carried_sprites() {
    ...
    frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (6 + animFrame)) / 2);
}

void Caveman::apply_dead_carried_sprites() {
    animFrame = 0;
    frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (0)) / 2);
}

void Caveman::apply_dead_sprites() {
    animFrame = 0;
    if (ySpeed == 0)
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (1)) / 2);
    else if (ySpeed > 0)
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (4)) / 2);
    else
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (2)) / 2);
...
void Caveman::apply_walking_sprites() {
    ...
    if (xSpeed == 0 && ySpeed == 0)
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (10)) / 2);
    else
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (animFrame + 11)) / 2);
...
void Caveman::apply_stunned_sprites() {
    ...
    if (ySpeed == 0)
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (animFrame + 15)) / 2);
    else if (ySpeed > 0)
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (4)) / 2);
    else
        frameGfx = (u8 *) gfx_cavemanTiles + ((sprite_width * sprite_height * (2)) / 2);
